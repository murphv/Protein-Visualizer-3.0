    ...
    if (isWindowView) {
      ...
      leftBonds.forEach((pair, idx) => {
        ...
        bond
          ...
          .attr('x2', bondPos)
          .attr('y2', 1.10 * bondHeight(idx))
          .style('stroke', 'black');

        const sulfide = g.append('text');
        sulfide
          .attr('dx', bondPos - 5)
          .attr('dy', 1.10 *bondHeight(idx) + SULFIDE_ATOM_OFFSET)
          .text(() => 'S')
        ...
        pos
          .attr('dx', bondPos + 6)
          .attr('dy', 1.10 *bondHeight(idx) + SULFIDE_ATOM_OFFSET + 5)
          .text(() => `${y}`)
